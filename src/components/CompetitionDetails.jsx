...
import { calculateStandings, getChampionIds, generateCopaRecompensaBracket, generateGroupsAndKnockout, generateCupBracket, generateRoundRobin } from '../utils/torneios';
...
const CompetitionDetails = ({ comp, teams, matches, competitions = [], users = [], onBack, currentUser, onReleaseRound, onLockRound, onSelectMatch, onDeleteMatch, onEditComp, showToast, onUpdatePlayedMatch, onSubmitMatch, onUpdateMatchStatus, onBatchUpdateComp }) => {
  ...
  const [settingsData, setSettingsData] = useState({
    ...
    isIdaEVolta: comp?.isIdaEVolta || false
  });
  ...
  const CATEGORY_NAMES = { liga_a: '🥇 Liga Kame A', liga_b: '🥈 Liga Kame B', liga_c: '🥉 Liga Kame C', liga_d: '🎖️ Liga Kame D', liga_acesso: ' ⬆️ Liga de acesso', copa_estrelas: '⭐ Copa das Estrelas', copa_main: '🏆 Copa Oficial', copa_flash: '⚡ Copa Flash Solo', copa_flash_dupla: '👥 Copa Flash Duplas', copa_do_rei: '👑 Copa do Rei', copa_amazonia: '🌳 Copa da Amazônia', copa_recompensa: '🎁 Copa Recompensa' };
  ...
  let timeLeft = 0; let isExpired = false;
  if (isFlash && activeRound && comp.flashDuration) { const deadline = (activeRound.releasedAt || Date.now()) + (comp.flashDuration * 60000); timeLeft = deadline - now; isExpired = timeLeft <= 0; }

  const formatTime = (ms) => { if (ms <= 0) return '00:00'; const totalSecs = Math.floor(ms / 1000); const m = Math.floor(totalSecs / 60).toString().padStart(2, '0'); const s = (totalSecs % 60).toString().padStart(2, '0'); return `${m}:${s}`; };
  ...
  const handleWithdrawTeam = () => {
    ...
    const newMatchDocs = []; const newRounds = JSON.parse(JSON.stringify(comp.rounds || []));
        ...
        round.matches.forEach(m => {
            if (m.teamA === withdrawTeamId || m.teamB === withdrawTeamId) {
                ...
                if (!isPlayed && m.teamA && m.teamB && !m.teamA.includes('Definir') && !m.teamB.includes('Definir')) {
                    ...
                    newMatchDocs.push({ id: `m_wo_quit_${Date.now()}_${Math.floor(Math.random()*10000)}`, compId: comp.id, roundId: round.id, matchId: m.id, teamA: m.teamA, teamB: m.teamB, scoreA: isTeamA ? 0 : 3, scoreB: isTeamA ? 3 : 0, penaltiesA: null, penaltiesB: null, goals: [], observacoes: '🏳️ W.O. por Desistência (Suspenso Próxima Edição)', status: 'pending', submittedBy: 'Sistema Admin' });
                }
            }
        });
    });
    onBatchUpdateComp({ ...comp, suspendedTeams: newSuspended, rounds: newRounds }, newMatchDocs); showToast(`Desistência registrada! ${newMatchDocs.length} W.O(s) gerados.`, "success"); setWithdrawTeamId('');
  };
  ...
  const handleRemoveTeamEntirely = () => {
    ...
    const newTeams = (comp.teams || []).filter(id => id !== withdrawTeamId); onEditComp({ ...comp, teams: newTeams }); showToast("Time removido da tabela com sucesso!", "success"); setWithdrawTeamId('');
  };
  ...
  const handleRemoveSuspension = (teamId) => {
    ...
    const newSuspended = (comp.suspendedTeams || []).filter(id => id !== teamId); onEditComp({ ...comp, suspendedTeams: newSuspended }); showToast("Banimento removido! A equipe está com a ficha limpa.", "success");
  };

  const handleAutoFlashRound = (round) => {
    if(!window.confirm('Encerrar fase? Sorteio automático (W.O Duplo) será aplicado nos jogos que não foram disputados.')) return; showToast('Processando...', 'info');
    const newMatchDocs = []; const matchResults = {}; 
    round.matches.forEach(m => {
        const sUI = matches.find(x => x.matchId === m.id && x.compId === comp.id && x.status === 'approved');
        if (sUI) {
            let wId = null; if (sUI.scoreA > sUI.scoreB) wId = m.teamA; else if (sUI.scoreB > sUI.scoreA) wId = m.teamB; else if (sUI.penaltiesA > sUI.penaltiesB) wId = m.teamA; else if (sUI.penaltiesB > sUI.penaltiesA) wId = m.teamB;
            matchResults[m.id] = wId;
        } else {
            if (m.teamA && m.teamB) {
                const winnerIsA = Math.random() < 0.5; const scoreA = winnerIsA ? 3 : 0; const scoreB = winnerIsA ? 0 : 3; const winnerId = winnerIsA ? m.teamA : m.teamB; matchResults[m.id] = winnerId;
                newMatchDocs.push({ id: `m_flash_${Date.now()}_${Math.floor(Math.random()*10000)}`, compId: comp.id, roundId: round.id, matchId: m.id, teamA: m.teamA, teamB: m.teamB, scoreA, scoreB, penaltiesA: null, penaltiesB: null, goals: [], observacoes: '⚡ W.O. Duplo Automático (Fim do Prazo)', status: 'approved', submittedBy: 'Sistema Flash' });
            } else if (m.teamA) { matchResults[m.id] = m.teamA; } else if (m.teamB) { matchResults[m.id] = m.teamB; }
        }
    });
    const newRounds = JSON.parse(JSON.stringify(comp.rounds)); const rIndex = newRounds.findIndex(r => r.id === round.id);
    if (rIndex >= 0 && rIndex < newRounds.length - 1) {
        const nextRound = newRounds[rIndex + 1];
        round.matches.forEach((m, mIdx) => {
           const winnerId = matchResults[m.id]; if (!winnerId) return;
           const nextMIndex = Math.floor(mIdx / 2); const isTeamA = mIdx % 2 === 0;
           ...
           if (isNextRoundFinal) {
              ...
              nextRound.matches.forEach(nextMatch => {
                 if (nextMatch.id.includes('_3rd')) { if (isTeamA) nextMatch.teamA = loserId; else nextMatch.teamB = loserId; } else if (nextMatch.id.includes('_f1')) { if (isTeamA) nextMatch.teamA = winnerId; else nextMatch.teamB = winnerId; }
              });
           } else { if (nextRound.matches[nextMIndex]) { if (isTeamA) nextRound.matches[nextMIndex].teamA = winnerId; else nextRound.matches[nextMIndex].teamB = winnerId; } }
        });
        nextRound.status = 'released'; nextRound.releasedAt = Date.now(); 
    }
    newRounds[rIndex].status = 'locked';
    if (onBatchUpdateComp) { onBatchUpdateComp({ ...comp, rounds: newRounds }, newMatchDocs); }
  };
  ...
  const { topScorers, topAssists } = useMemo(() => {
    ...
    (matches || []).filter(m => m.compId === comp.id && m.status === 'approved').forEach(m => {
      (m.goals || []).forEach(g => {
        if (g.player) { const pKey = g.player.trim().toLowerCase() + '_' + g.teamId; if(!scorers[pKey]) scorers[pKey] = { player: g.player, teamId: g.teamId, count: 0 }; scorers[pKey].count += 1; }
        if (g.assist) { const aKey = g.assist.trim().toLowerCase() + '_' + g.teamId; if(!assists[aKey]) assists[aKey] = { player: g.assist, teamId: g.teamId, count: 0 }; assists[aKey].count += 1; }
      });
    });
    return { topScorers: Object.values(scorers).sort((a,b) => b.count - a.count).slice(0, 15), topAssists: Object.values(assists).sort((a,b) => b.count - a.count).slice(0, 15) };
  }, [matches, comp.id]);
  ...
  const handleDeleteRound = (roundId) => {
    if (!window.confirm("⚠️ ATENÇÃO: Tem certeza que deseja excluir esta rodada?")) return;
    const updatedRounds = comp.rounds.filter(r => r.id !== roundId); onEditComp({ ...comp, rounds: updatedRounds }); showToast("Rodada removida com sucesso!", "success");
  };

  const handleOpenEditGroups = () => {
    const mapping = {}; if (comp.groups) Object.keys(comp.groups).forEach(gName => { (comp.groups[gName] || []).forEach(tId => { mapping[tId] = gName; }); });
  ...
  const handleAutoMigrateKnockout = () => {
    if (!comp.groups) return; showToast("Calculando classificados...", "info"); const qualifiers = {};
    Object.keys(comp.groups).forEach((gName) => {
      const gTeams = (teams || []).filter(t => comp.groups[gName].includes(t.id)); const gTable = calculateStandings(matches, gTeams, comp.id);
      gTable.forEach((row, idx) => { qualifiers[`${idx + 1}º Grupo ${gName}`] = row.id; qualifiers[`${idx + 1}º do Grupo ${gName}`] = row.id; qualifiers[`${idx + 1}º Gr.${gName}`] = row.id; });
    });
  ...
  const handleAddMatchToGroup = (roundId, groupLetter) => {
    ...
    const updatedRounds = comp.rounds.map(r => r.id === roundId ? { ...r, matches: [...r.matches, newMatch] } : r); onEditComp({ ...comp, rounds: updatedRounds }); showToast("Nova partida adicionada!", "success");
  };
  ...
  const handleAddNewGroup = (roundId) => {
    ...
    if (novoG && novoG.trim()) {
      ...
      const updatedRounds = comp.rounds.map(r => r.id === roundId ? { ...r, matches: [...r.matches, newMatch] } : r); onEditComp({ ...comp, rounds: updatedRounds, groups: updatedGroups }); showToast(`Grupo ${upperG} criado!`, "success");
    }
  };
  ...
  const handleAddTeamToComp = () => { 
    ...
    const newTeams = [...(comp.teams || []), newTeamToAdd]; const newPending = (comp.pendingTeams || []).filter(p => p.teamId !== newTeamToAdd); const newSuspended = (comp.suspendedTeams || []).filter(id => id !== newTeamToAdd);
    onEditComp({ ...comp, teams: newTeams, pendingTeams: newPending, suspendedTeams: newSuspended }); setNewTeamToAdd(''); setShowAddTeam(false); showToast("Time inserido e banimento removido com sucesso!", "success"); 
  };
  ...
  const groupOrNormalRounds = (comp.rounds || []).filter(r => !r.id.includes('ko') && comp.format !== 'cup' && comp.category !== 'copa_flash_dupla' && comp.category !== 'copa_recompensa');
  ...
  if (comp.status === 'drawing') {
    if (isAdmin) {
      return (
        <LiveDrawPanel comp={comp} teams={teams} matches={matches} onCancel={() => onEditComp({ ...comp, status: 'registration' })} onFinish={(rounds, duplasResult) => { onEditComp({ ...comp, status: 'active', rounds: rounds, groups: duplasResult }); showToast("Sorteio Concluído! A Tabela Oficial foi gerada.", "success"); }} />
      );
    }
    return (
      <div className="space-y-6 animate-in fade-in pb-10"><button onClick={onBack} className="flex items-center gap-1.5 text-xs text-blue-400 hover:text-white"><ArrowLeft size={16}/> Voltar</button><div className="bg-blue-900 p-8 md:p-12 rounded-3xl border border-amber-500/50 flex flex-col items-center text-center shadow-2xl mt-8"><span className="text-6xl mb-6 animate-bounce">🎲</span><h2 className="text-3xl md:text-4xl font-black text-amber-400 uppercase tracking-widest mb-4">Sorteio Ao Vivo Ativo</h2><p className="text-blue-300 md:w-2/3 leading-relaxed">As inscrições foram encerradas. Acompanhe a live dos líderes!<br/><br/>A tabela e os confrontos estão bloqueados enquanto a diretoria realiza o sorteio oficial ao vivo.</p></div></div>
    );
  }
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      ...
      <div className="bg-blue-900 p-5 rounded-3xl border border-blue-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-xl">
        <div>
          ...
          <div className="flex items-center flex-wrap gap-2 mt-2">
            ...
            {comp.isIdaEVolta && <span className="text-xs text-amber-400 font-bold ml-1">• Ida e Volta ⚔️</span>}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1"><label className="text-xs font-bold text-blue-400">Categoria (Ranking)</label>
              <select value={settingsData.category} onChange={e => setSettingsData({...settingsData, category: e.target.value})} className="w-full bg-blue-900 border border-blue-700 rounded-lg p-2.5 text-white text-sm outline-none focus:border-emerald-500">
                <option value="liga_a">🥇 Liga Kame A (Série A)</option><option value="liga_b">🥈 Liga Kame B (Série B)</option><option value="liga_c">🥉 Liga Kame C (Série C)</option><option value="liga_d">🎖️ Liga Kame D (Série D)</option><option value="liga_acesso">⬆️ Liga de Acesso</option><option value="copa_main">🏆 Copas Oficiais (Ex: Copa do Clã)</option><option value="copa_do_rei">👑 Copa do Rei</option><option value="copa_estrela"> ⭐ Copa das Estrelas</option><option value="copa_amazonia">🌳 Copa da Amazônia</option><option value="copa_flash">⚡ Copa Flash Solo</option><option value="copa_flash_dupla">👥 Copa Flash Duplas</option><option value="copa_recompensa">🎁 Copa Recompensa</option>
              </select>
            </div>
            <div className="space-y-1"><label className="text-xs font-bold text-blue-400">Estilo de Jogo</label><select value={settingsData.playStyle} onChange={e => setSettingsData({...settingsData, playStyle: e.target.value})} className="w-full bg-blue-900 border border-blue-700 rounded-lg p-2.5 text-white text-sm outline-none focus:border-emerald-500"><option value="Livre">Livre (Qualquer Estilo)</option><option value="Full Razz">Full Razz (Sem Balão)</option><option value="Personalizado">Regras Personalizadas</option></select></div>
            ...
            <div className="space-y-1 md:col-span-2 mt-2">
              <label className="flex items-center gap-2 cursor-pointer bg-blue-900 p-3 rounded-xl border border-blue-700 hover:border-amber-500 transition-colors">
                ...
                <span className="text-sm font-bold text-amber-400">☑️ Confrontos de Ida e Volta (Mata-Mata)</span>
              </label>
              <p className="text-[10px] text-blue-400 mt-1 ml-1">Se ativado, todas as fases do mata-mata terão dois jogos. O sistema calculará o Placar Agregado automaticamente.</p>
            </div>

            {isLeader && (<div className="space-y-2 md:col-span-2 pt-2 border-t border-blue-800"><label className="text-xs font-bold text-emerald-400">Adicionar Organizadores</label><div className="flex flex-wrap gap-2">{users.filter(u => u.role === 'organizer').map(u => ( <label key={u.id} className="flex items-center gap-2 text-xs text-blue-200 bg-blue-950 px-3 py-2 rounded-lg border border-blue-800 cursor-pointer hover:border-emerald-500/50"><input type="checkbox" checked={settingsData.admins.includes(u.id)} onChange={e => { const newAdmins = e.target.checked ? [...settingsData.admins, u.id] : settingsData.admins.filter(id => id !== u.id); setSettingsData({...settingsData, admins: newAdmins}); }} className="accent-emerald-500 w-3 h-3" /> {u.name}</label> ))}</div></div>)}
            {isLeader && (<div className="space-y-2 md:col-span-2 pt-2 border-t border-blue-800"><label className="text-xs font-bold text-red-400 flex items-center gap-1.5"><XCircle size={14}/> Bloquear inscritos dos torneios:</label><div className="flex flex-wrap gap-2">{competitions.filter(c => c.status === 'active' && c.id !== comp.id).map(c => ( <label key={c.id} className="flex items-center gap-1.5 text-xs text-blue-200 bg-blue-950 px-3 py-2 rounded-lg border border-blue-800 cursor-pointer hover:border-red-500/50 transition-colors"><input type="checkbox" checked={settingsData.excludedCompIds.includes(c.id)} onChange={e => { const newEx = e.target.checked ? [...settingsData.excludedCompIds, c.id] : settingsData.excludedCompIds.filter(id => id !== c.id); setSettingsData({...settingsData, excludedCompIds: newEx}); }} className="accent-red-500 w-3 h-3" /> {c.name}</label> ))}</div></div>)}
          </div>
          <div className="flex justify-end gap-2 pt-2"><button onClick={() => setShowEditSettings(false)} className="px-4 py-2 bg-blue-900 border border-blue-700 rounded-lg text-xs text-blue-300 hover:text-white">Cancelar</button><button onClick={() => { const cleanCatName = CATEGORY_NAMES[settingsData.category] || 'Competição'; onEditComp({ ...comp, name: settingsData.edition ? `${cleanCatName.split(' ').slice(1).join(' ')} - Edição ${settingsData.edition}` : comp.name, category: settingsData.category, playStyle: settingsData.playStyle, rules: settingsData.rules, promotions: settingsData.promotions, relegations: settingsData.relegations, admins: settingsData.admins, excludedCompIds: settingsData.excludedCompIds, registrationStartTime: settingsData.registrationStartTime, isIdaEVolta: settingsData.isIdaEVolta }); setShowEditSettings(false); showToast("Configurações atualizadas!", "success"); }} className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg text-xs shadow-md">Salvar</button></div>
        </div>
      )}
      ...
      {showEditGroups && (
        <div className="bg-purple-900/30 border border-purple-500/50 p-5 rounded-2xl space-y-4 animate-in slide-in-from-top-4 shadow-xl mb-6">
          <h3 className="text-sm font-black text-purple-400 uppercase tracking-wider flex items-center gap-2"><Users size={16}/> Gerenciar Divisão de Grupos</h3>
          ...
          <div className="flex flex-col sm:flex-row gap-2 mt-4 pb-4 border-b border-purple-800/50"><input type="text" value={newGroupName} onChange={e => setNewGroupName(e.target.value)} placeholder="Letra/Nome de um Novo Grupo (Opcional)" className="flex-1 bg-blue-950 border border-purple-700/50 rounded-lg p-2 text-white text-xs outline-none focus:border-purple-400 uppercase" /><button onClick={() => { if(!newGroupName) return; const gn = newGroupName.trim().toUpperCase(); if(Object.values(teamGroupMapping).includes(gn)) { showToast("Esse grupo já existe!", "warning"); return; } const currentUnassigned = Object.keys(teamGroupMapping).find(k => !teamGroupMapping[k]); if (currentUnassigned) setTeamGroupMapping({...teamGroupMapping, [currentUnassigned]: gn}); setNewGroupName(''); showToast(`Grupo ${gn} disponibilizado para marcação!`, "success"); }} className="bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold py-2 px-4 rounded-lg shadow-md shrink-0">Criar Nova Coluna de Grupo</button></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mt-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">{compTeams.map(t => { const currentGroup = teamGroupMapping[t.id] || 'A'; const uniqueGroups = Array.from(new Set(Object.values(teamGroupMapping))); if (!uniqueGroups.includes('A')) uniqueGroups.push('A'); return ( <div key={t.id} className="bg-blue-950 p-3 rounded-xl border border-blue-800 flex flex-col gap-2"><div className="flex items-center gap-2 min-w-0"><ShieldDisplay shield={t.shield} size="small" /><span className="font-bold text-xs text-blue-100 truncate">{t.name}</span></div><select value={currentGroup} onChange={e => setTeamGroupMapping({...teamGroupMapping, [t.id]: e.target.value})} className="w-full bg-blue-900 border border-purple-500/30 rounded p-1.5 text-purple-300 text-xs font-bold outline-none cursor-pointer">{uniqueGroups.sort((a,b)=>a.localeCompare(b)).map(g => ( <option key={g} value={g}>Grupo {g}</option> ))}</select></div> ); })}</div>
          <div className="flex justify-end gap-2 pt-4 border-t border-purple-800/50 mt-4"><button onClick={() => setShowEditGroups(false)} className="px-4 py-2 bg-blue-950 border border-purple-700/50 rounded-lg text-xs text-purple-300 hover:text-white transition-colors">Cancelar</button><button onClick={handleSaveGroups} className="px-5 py-2 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg text-xs shadow-md transition-colors">💾 Salvar Divisão</button></div>
        </div>
      )}
      ...
      {isRegistration ? (
        <div className="bg-blue-900 border border-blue-800 rounded-3xl p-6 md:p-8 shadow-2xl animate-in slide-in-from-bottom-4">
          <div className="text-center mb-8">
            ...
            <div className="mt-6 flex flex-col items-center justify-center gap-4"><div className="bg-blue-950 px-8 py-4 rounded-2xl border border-blue-800 shadow-inner"><p className="text-xs text-blue-400 uppercase font-bold mb-1">Vagas Preenchidas</p><p className="text-4xl font-black text-white">{(comp.teams?.length || 0)} <span className="text-blue-600 text-2xl">/ {comp.teamCount}</span></p></div><Button onClick={handleCopyLink} className="py-3 px-8 text-sm font-bold bg-blue-600 shadow-xl">🔗 Copiar Link de Inscrição</Button></div>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div>
              ...
              <div className="space-y-3">
                ...
                {(comp.pendingTeams || []).map((req, idx) => { const t = getTeam(req.teamId); return ( <div key={idx} className="bg-blue-950 p-4 rounded-xl border border-amber-500/30 flex flex-col gap-3"><div className="flex items-center gap-3"><ShieldDisplay shield={t?.shield} size="small" /><div><p className="font-bold text-white text-sm">{t?.name}</p><p className="text-[10px] text-blue-400">Técnico: {t?.coach}</p></div></div><div className="flex gap-2"><Button variant="outline" onClick={()=>handleRejectTeam(req)} className="flex-1 py-2 text-xs text-red-400">Recusar</Button><Button onClick={()=>handleApproveTeam(req)} className="flex-1 py-2 text-xs bg-emerald-600">Aprovar</Button></div></div> ); })}
              </div>
            </div>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                {(comp.teams || []).map(tId => { const t = getTeam(tId); return (<div key={tId} className="bg-blue-950 p-3 rounded-xl border border-emerald-500/20 flex items-center justify-between gap-2 group cursor-pointer" onClick={() => setSelectedTeamHistory(tId)}><div className="flex items-center gap-2 min-w-0"><ShieldDisplay shield={t?.shield} size="small" /><span className="font-bold text-xs text-blue-100 truncate hover:text-emerald-400 transition-colors">{t?.name}</span></div>{isAdmin && ( <button onClick={(e) => { e.stopPropagation(); handleRemoveConfirmedTeam(tId); }} className="text-blue-500 hover:text-red-400 p-1 md:opacity-0 group-hover:opacity-100 transition-opacity" title="Remover Time"><X size={14} /></button> )}</div>); })}
              </div>
            </div>
          </div>
          <div className="mt-8 pt-8 border-t border-blue-800"><Button onClick={handleGenerateBracket} disabled={comp.teams?.length !== comp.teamCount} className="w-full py-5 text-xl font-black rounded-2xl bg-emerald-500 text-blue-950 hover:bg-emerald-400 disabled:bg-blue-900 disabled:text-blue-700 shadow-2xl">🏆 Encerrar Inscrições e Gerar Tabela</Button></div>
        </div>
      ) : (
        <>
          <div className="flex gap-1 p-1 bg-blue-950 rounded-xl border border-blue-800 overflow-x-auto custom-scrollbar">
            ...
            {isAdmin && (<><button onClick={()=>setSubTab('submit')} className={`shrink-0 flex-1 px-4 py-1.5 text-xs rounded-lg font-bold transition-all ${subTab==='submit'?'bg-emerald-600 text-white':'text-blue-500 hover:text-white'}`}>Registrar</button><button onClick={()=>setSubTab('validation')} className={`shrink-0 flex-1 px-4 py-1.5 text-xs rounded-lg font-bold transition-all flex justify-center items-center gap-1 ${subTab==='validation'?'bg-amber-600 text-white':'text-amber-500/70 hover:text-amber-400'}`}>Validação {matches.filter(m => m.compId === comp.id && m.status === 'pending').length > 0 && <span className="bg-red-500 text-white px-1.5 py-0.5 rounded-full text-[9px] shadow-sm">{matches.filter(m => m.compId === comp.id && m.status === 'pending').length}</span>}</button><button onClick={()=>setSubTab('draw')} className={`shrink-0 flex-1 px-4 py-1.5 text-xs rounded-lg font-bold transition-all flex justify-center items-center gap-1 ${subTab==='draw'?'bg-purple-600 text-white':'text-purple-400 hover:text-purple-300'}`}>🎁 Sorteio</button></>)}
          </div>
          ...
          <div className="space-y-8 mt-4">
            {subTab === 'overview' && (
              <div className="space-y-6 animate-in slide-in-from-left-4">
                ...
                {comp.rules && (<div className="bg-blue-950/80 p-5 rounded-2xl border border-blue-800 shadow-inner"><h4 className="text-sm font-bold text-sky-400 mb-2 flex items-center gap-2"><BookOpen size={16}/> Regras do Torneio</h4><p className="text-xs text-blue-200 whitespace-pre-wrap leading-relaxed">{comp.rules}</p></div>)}
                ...
                {isAdmin && comp.status === 'active' && (
                  <div className="bg-red-950/40 p-5 rounded-xl border border-red-800 shadow-inner mb-6 animate-in slide-in-from-top-2">
                    <h4 className="text-sm font-bold text-red-400 mb-2 flex items-center gap-2"><Trash2 size={16}/> Gerenciar Saída de Equipe</h4>
                    <div className="text-[10px] text-red-200 mb-4 space-y-1"><p><b>⚠️ Banimento (W.O):</b> Mantém o time na tabela. Dá vitória de 3x0 para os adversários restantes e suspende para a próxima edição.</p><p><b>❌ Remover da Competição:</b> Apaga o time da tabela de classificação como se ele nunca tivesse entrado.</p></div>
                    <div className="flex flex-col sm:flex-row gap-2"><select value={withdrawTeamId} onChange={e=>setWithdrawTeamId(e.target.value)} className="flex-1 bg-blue-900 border border-red-500/50 rounded-lg p-2 text-white text-xs outline-none focus:border-red-400"><option value="">Selecione a equipe...</option>{compTeams.map(t => (<option key={t.id} value={t.id}>{t.name}</option>))}</select><button onClick={handleWithdrawTeam} className="bg-red-600 hover:bg-red-500 text-white font-bold text-xs py-2 px-3 rounded-lg shadow-lg transition-colors shrink-0">Aplicar Banimento (W.O)</button><button onClick={handleRemoveTeamEntirely} className="bg-blue-950 hover:bg-blue-900 border border-red-800 text-red-400 hover:text-red-300 font-bold text-xs py-2 px-3 rounded-lg shadow-lg transition-colors shrink-0">Remover da Tabela</button></div>
                    {(comp.suspendedTeams || []).length > 0 && (<div className="mt-4 pt-3 border-t border-red-800/50"><span className="text-[10px] text-red-400 font-bold uppercase tracking-widest block mb-2">Equipes Banidas Deste Torneio:</span><div className="flex flex-wrap gap-2">{comp.suspendedTeams.map(id => { const t = getTeam(id); return ( <span key={id} className="bg-red-500/10 text-red-300 border border-red-500/30 pl-2 pr-1 py-1 rounded text-[10px] flex items-center gap-1.5 transition-colors hover:bg-red-500/20"><Lock size={10}/> {t?.name || 'Desconhecido'}<button onClick={() => handleRemoveSuspension(id)} className="bg-red-900/50 hover:bg-red-500 text-red-400 hover:text-white p-0.5 rounded ml-1 transition-colors" title="Remover Banimento"><X size={12}/></button></span> ); })}</div></div>)}
                  </div>
                )}
                
                {comp.format !== 'league' && (
                  <div className="flex justify-center"><div className="bg-blue-950 p-1 rounded-xl border border-blue-800 flex gap-1"><button type="button" onClick={() => setViewType('bracket')} className={`px-4 py-1.5 text-xs rounded-lg font-bold transition-colors ${viewType === 'bracket' ? 'bg-amber-600 text-white' : 'text-blue-400 hover:text-white'}`}>🏆 Chaveamento Mata-Mata</button><button type="button" onClick={() => setViewType('table')} className={`px-4 py-1.5 text-xs rounded-lg font-bold transition-colors ${viewType === 'table' ? 'bg-amber-600 text-white' : 'text-blue-400 hover:text-white'}`}>{comp.format === 'groups' ? '📋 Classificação dos Grupos' : '📋 Tabela Geral Tradicional'}</button></div></div>
                )}
                ...
                {isFlash && activeRound && (
                  <div className="bg-gradient-to-r from-amber-900/40 to-blue-900/40 border border-amber-500/50 p-5 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4 shadow-xl relative overflow-hidden animate-in slide-in-from-top-4">
                     ...
                     <div><h4 className="text-amber-400 font-black uppercase tracking-widest flex items-center gap-2"><Activity size={18}/> MODO COPA FLASH ⚡</h4><p className="text-xs text-amber-100/70 mt-1">Fase Atual: <b>{activeRound.number}</b> • Todos os jogos devem ser enviados antes do tempo esgotar!</p></div>
                     <div className="flex flex-col items-center md:items-end gap-2 shrink-0"><div className="bg-blue-950 px-4 py-2 rounded-xl border border-blue-800 shadow-inner"><span className="text-[10px] uppercase font-bold text-blue-400 block mb-1 text-center md:text-right">Tempo Restante</span><span className={`text-3xl font-mono font-black tracking-wider ${isExpired ? 'text-red-500 animate-pulse' : 'text-emerald-400'}`}>{formatTime(timeLeft)}</span></div>{isAdmin && (<button onClick={() => handleAutoFlashRound(activeRound)} className={`px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-wider transition-all shadow-lg ${isExpired ? 'bg-amber-600 hover:bg-amber-500 text-white animate-bounce' : 'bg-blue-800 hover:bg-amber-600 text-blue-300 hover:text-white border border-blue-700 hover:border-amber-500'}`}>⚡ Encerrar Fase Automático</button>)}</div>
                  </div>
                )}
                              ...
                              return (
                                <div key={round.id} className={`bg-blue-900 border rounded-xl overflow-hidden ${isLocked ? 'border-blue-800/50' : 'border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.05)]'}`}>
                                  <div className="w-full bg-blue-950/60 flex items-center justify-between pr-4">
                                    <button type="button" onClick={() => toggleRound(round.id)} className="flex-1 p-4 flex justify-between items-center outline-none"><span className={`text-sm font-bold flex items-center gap-2 ${isLocked ? 'text-blue-400' : 'text-emerald-400'}`}>{isLocked ? <Lock size={16} className="text-amber-500"/> : <PlayCircle size={16} className="text-emerald-500"/>} Rodada {round.number}</span><span className="text-blue-500 text-xs font-bold mr-2">{isExpanded ? '▲ Recolher' : '▼ Expandir'}</span></button>
                                    <div className="flex items-center gap-1.5">{isAdmin && isLocked && (<button type="button" onClick={(e) => { e.stopPropagation(); onReleaseRound(comp.id, round.id); }} className="bg-emerald-600 hover:bg-emerald-500 text-blue-950 font-black text-[10px] px-3 py-1.5 rounded uppercase tracking-wider transition-colors shrink-0 shadow-md">🔓 Liberar</button>)}{isAdmin && !isLocked && (<button type="button" onClick={(e) => { e.stopPropagation(); onLockRound(comp.id, round.id); }} className="bg-amber-600 hover:bg-amber-500 text-blue-950 font-black text-[10px] px-3 py-1.5 rounded uppercase tracking-wider transition-colors shrink-0 shadow-md">🔒 Travar</button>)}{isAdmin && (<button type="button" onClick={(e) => { e.stopPropagation(); handleDeleteRound(round.id); }} className="bg-red-500/20 hover:bg-red-500 text-red-400 hover:text-white p-1.5 rounded transition-colors shrink-0 shadow-md" title="Excluir Rodada"><Trash2 size={14}/></button>)}</div>
                                  </div>
                                  {isExpanded && (
                        ...
                        <div className="flex gap-12 items-stretch pb-8 min-w-max px-6 pt-4">
                          {knockoutRounds.map((round, roundIndex) => {
                            ...
                            if (comp.category === 'copa_recompensa') {
                                ...
                                else if (distanceToFinal === 4) phaseName = "PLAYOFF DE ACESSO";
                                else if (distanceToFinal === 5) phaseName = "FASE 2";
                                else if (distanceToFinal === 6) phaseName = "FASE 1";
                            } else {
                                if (distanceToFinal === 1) phaseName = "FINAL"; 
                                else if (distanceToFinal === 2) phaseName = "SEMIFINAL"; 
                                else if (distanceToFinal === 3) phaseName = "QUARTAS"; 
                                  ...
                                  {comp.category === 'copa_flash_dupla' || comp.isIdaEVolta ? (
                                    Array.from({ length: Math.ceil(round.matches.length / 2) }).map((_, idx) => {
                                      ...
                                      if (!mVolta || mIda.id.includes('_3rd') || (!mIda.id.includes('_ida') && !mIda.id.includes('_f1'))) {
                                          ...
                                          if (isPlayed && !isBye) { const scoreA = Number(sUI.scoreA || 0); const scoreB = Number(sUI.scoreB || 0); if (scoreA < scoreB) teamALost = true; else if (scoreB < scoreA) teamBLost = true; else { const penA = Number(sUI.penaltiesA||0); const penB = Number(sUI.penaltiesB||0); if (penA < penB) teamALost = true; if (penB < penA) teamBLost = true; } }
                                          return (
                                            <div key={mIda.id} className="relative flex-1 flex flex-col justify-center py-3 group">
                                              <div className="relative z-10 w-full">
                                                <div onClick={() => { if(sUI.isPlayed && onSelectMatch && !isBye){ const f = matches.find(x=>x.id===sUI.submittedMatchId); if(f) onSelectMatch(f) } }} className={`p-3 rounded-xl border flex flex-col gap-1.5 transition-all shadow-sm ${sUI.isPlayed || isBye ? 'bg-blue-900/90 border-emerald-500/30' : isLocked ? 'bg-blue-950/40 border-blue-900/60 opacity-40' : 'bg-blue-900/40 border-blue-800 hover:border-blue-600'} ${!isBye ? 'cursor-pointer' : ''} relative overflow-hidden`}>
                                                  <div className="flex justify-between items-center text-[9px] font-bold uppercase tracking-wider pb-1 border-b border-blue-800/40">
                                                    <span className="text-blue-500">{mIda.id.includes('_3rd') ? '🥉 Disputa 3º Lugar' : 'Confronto'}</span><span className={isBye ? 'text-emerald-400' : sUI.color}>{isBye ? 'Avanço Direto' : sUI.text}</span>
                                                  </div>
                                      ...
                                      return (
                                        <div key={`dupla_${idx}`} className="relative flex-1 flex flex-col justify-center py-3 group">
                                          ...
                                          <div className="relative z-10 w-full">
                                            ...
                                            >
                                              <div className="flex justify-between items-center text-[9px] font-bold uppercase tracking-wider pb-1 border-b border-blue-800/40">
                                                <span className="text-amber-400 flex items-center gap-1">{isDupla ? '👥 Confronto Duplo' : '⚔️ Ida e Volta'}</span>
                                              ...
                                              <div className={`flex items-center justify-between gap-2 min-w-0 mt-1 transition-all ${teamALost ? 'grayscale opacity-50 line-through' : ''}`}>
                                                <span className={`text-xs truncate font-bold ${isPlayed && !teamALost ? 'text-emerald-400' : 'text-blue-100'}`}>{idaIsFinished || isFirstRound || isAdmin ? (realTeamA?.name || mIda.placeholderA) : 'Oculto 🕵️'}</span>
                                              ...
                                              <div className={`flex items-center justify-between gap-2 min-w-0 transition-all ${teamBLost ? 'grayscale opacity-50 line-through' : ''}`}>
                                                <span className={`text-xs truncate font-bold ${isPlayed && !teamBLost ? 'text-emerald-400' : 'text-blue-100'}`}>{idaIsFinished || isFirstRound || isAdmin ? (realTeamB?.name || mIda.placeholderB) : 'Oculto 🕵️'}</span>
            ...
            {subTab === 'stats' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-right-4">
                <div className="space-y-2">
                  ...
                  <div id="capture-scorers" className="bg-blue-900 rounded-xl border border-blue-800 overflow-hidden shadow-xl p-2 sm:p-4"><div className="bg-blue-950/80 p-4 border border-blue-800 rounded-xl mb-4 flex flex-col items-center justify-center"><h3 className="font-bold text-emerald-400 text-lg uppercase tracking-widest text-center">⚽ Artilharia</h3><span className="text-[10px] font-bold text-blue-400 mt-1">{comp.name}</span></div>
                    <div className="divide-y divide-blue-800/50 bg-blue-950 rounded-xl border border-blue-800">
                      {topScorers.length === 0 ? <p className="p-6 text-sm text-blue-500 text-center">Nenhum gol validado até o momento.</p> : (
                        topScorers.map((s, idx) => (
                          <div key={idx} className="p-3 flex items-center justify-between hover:bg-blue-800/50 transition-colors">
                            <div className="flex items-center gap-3"><span className={`font-black w-6 text-center ${idx === 0 ? 'text-amber-400 text-lg' : idx === 1 ? 'text-blue-300 text-lg' : idx === 2 ? 'text-amber-700 text-lg' : 'text-blue-600'}`}>{idx + 1}º</span><ShieldDisplay shield={getTeam(s.teamId)?.shield} size="normal" /><div className="flex flex-col"><span className="font-bold text-blue-200 text-sm md:text-base leading-tight">{s.player}</span><span className="text-[10px] md:text-xs text-blue-400 font-medium">{getTeam(s.teamId)?.name}</span></div></div><div className="bg-blue-900 px-4 py-2 rounded-lg border border-blue-800 text-emerald-400 font-black text-lg">{s.count}</div>
                          </div>
                        ))
                      )}
                    </div>
      ...
      {/* 👥 MODAL DETALHES DA DUPLA / IDA E VOLTA */}
            ...
            <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in" onClick={() => setSelectedDuplaMatchup(null)}>
              <div className="bg-blue-900 border border-blue-700 rounded-2xl w-full max-w-md p-6 shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex justify-between items-center mb-6 border-b border-blue-800 pb-4">
                  <h3 className="text-lg font-black text-amber-400 uppercase tracking-widest flex items-center gap-2">{isDupla ? '👥 Confronto Duplo' : '⚔️ Ida e Volta'}</h3>
                ...
                <div className="space-y-4 mb-6">
                  <div>
                    <label className="text-[10px] font-bold text-blue-400 uppercase tracking-widest block mb-1">{isDupla ? 'Dupla 1' : 'Time 1'} (Mandante Ida)</label>
                    <input type="text" value={duplaA?.name || ''} disabled={!isAdmin || (isDupla && typeof duplaA === 'string')}
                      onChange={(e) => {
                          if(isDupla) {
                    ...
                    />
                    {isDupla && <p className="text-[9px] text-emerald-400 mt-1">Formação: {getTeam(duplaA?.p1)?.name || 'Oculto'} & {getTeam(duplaA?.p2)?.name || 'Oculto'}</p>}
                  </div>
                  <div className="text-center font-black text-blue-500 text-lg">X</div>
                  <div>
                    <label className="text-[10px] font-bold text-blue-400 uppercase tracking-widest block mb-1">{isDupla ? 'Dupla 2' : 'Time 2'} (Mandante Volta)</label>
                    <input type="text" value={duplaB?.name || ''} disabled={!isAdmin || (isDupla && typeof duplaB === 'string')}
                      onChange={(e) => {
                          if(isDupla) {
                      ...
                      className="w-full bg-blue-950 border border-blue-700 rounded p-2 text-white text-sm outline-none focus:border-amber-500 disabled:opacity-70" 
                    />
                    {isDupla && <p className="text-[9px] text-emerald-400 mt-1">Formação: {getTeam(duplaB?.p1)?.name || 'Oculto'} & {getTeam(duplaB?.p2)?.name || 'Oculto'}</p>}
                  </div>
                </div>
                ...
                {isAdmin && isPlayed && (comp.rounds.findIndex(r => r.id === roundId) < comp.rounds.length - 1) && (
                  <div className="mt-4 grid grid-cols-2 gap-3">
                      <button onClick={() => handleForceAdvanceDupla(roundId, mIda, duplaA)} className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2.5 rounded-lg shadow-md transition-colors truncate px-2">
                        Avançar {duplaA && typeof duplaA.name === 'string' ? duplaA.name.split(' ')[0] : (duplaA?.name || (isDupla ? 'Dupla 1' : 'Time 1'))}
                      </button>
                      <button onClick={() => handleForceAdvanceDupla(roundId, mIda, duplaB)} className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2.5 rounded-lg shadow-md transition-colors truncate px-2">
                        Avançar {duplaB && typeof duplaB.name === 'string' ? duplaB.name.split(' ')[0] : (duplaB?.name || (isDupla ? 'Dupla 2' : 'Time 2'))}
                      </button>
                  </div>
                )}
        ...
        (comp.rounds || []).forEach(round => {
          (round.matches || []).forEach(m => {
            if (m.teamA === selectedTeamHistory || m.teamB === selectedTeamHistory) {
              ...
              teamMatches.push({ roundNumber: round.number, match: m, statusUI: sUI, isTeamA: m.teamA === selectedTeamHistory });
            }
          });
        });
            ...
            <div className="bg-blue-900 border border-blue-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center mb-4 border-b border-blue-800 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="text-base sm:text-lg font-black text-white uppercase tracking-wider">{teamInfo?.name || 'Time'}</h3><p className="text-[10px] text-emerald-400 font-bold uppercase tracking-widest">Histórico na Competição</p></div>
                </div>
              ...
              <div className="overflow-y-auto custom-scrollbar space-y-3 pr-1 flex-1">
                {teamMatches.length === 0 ? (
                  ...
                  teamMatches.map((item, idx) => {
                    ...
                    const oppId = isTeamA ? match.teamB : match.teamA; const oppTeam = getTeam(oppId);
                    return (
                      <div key={idx} className="bg-blue-950 p-3.5 rounded-xl border border-blue-800 flex items-center justify-between gap-3 shadow-inner">
                        <div className="flex flex-col"><span className="text-[9px] font-black uppercase text-amber-400 tracking-wider">Rodada {roundNumber}</span><span className="text-xs font-bold text-blue-200 mt-0.5">vs {oppTeam?.name || match.placeholderB || match.placeholderA || 'Adversário'}</span></div>
                        <div className="flex items-center gap-3 shrink-0"><span className={`text-[9px] uppercase font-black px-2 py-0.5 rounded ${statusUI.bg} ${statusUI.color}`}>{statusUI.text}</span><div className="bg-blue-900 px-3 py-1 rounded-lg border border-blue-800 text-sm font-black text-white">{statusUI.isPlayed ? ( isTeamA ? `${statusUI.scoreA} : ${statusUI.scoreB}` : `${statusUI.scoreB} : ${statusUI.scoreA}` ) : ( <span className="text-blue-500 font-bold text-xs">x</span> )}</div></div>
                      </div>
                    );
                  })
                )}
              </div>
              <div className="mt-4 pt-3 border-t border-blue-800 text-center"><button onClick={() => setSelectedTeamHistory(null)} className="w-full py-2 bg-blue-800 hover:bg-blue-700 text-white font-bold text-xs rounded-xl transition-colors">Fechar Histórico</button></div>
            </div>
          </div>
        );
      })()}
