...
const inputClass = "w-full bg-slate-950 border border-slate-700 focus:border-blue-500 rounded-lg p-3 text-white outline-none transition-colors text-sm";
...
const Button = ({ children, onClick, variant = 'primary', className = '', disabled = false, type = 'button' }) => {
  const variants = { primary: "bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-900/50", secondary: "bg-slate-700 hover:bg-slate-600 text-white", danger: "bg-red-600 hover:bg-red-500 text-white", outline: "border border-slate-600 text-slate-300 hover:bg-slate-800" };
        ...
        {view === 'login' && (
          <form onSubmit={handleLoginSubmit} className="space-y-4 animate-in fade-in duration-300">
            ...
            <div className="text-center pt-5 border-t border-slate-800/50 mt-6">
              ...
              <button type="button" onClick={() => {setView('register'); setError('');}} className="text-sm font-bold text-blue-400 hover:text-blue-300 underline">Primeiro Acesso (Cadastrar)</button>
            </div>
          </form>
        )}
    ...
    <div className="max-w-2xl mx-auto animate-in fade-in pb-12">
      ...
      <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 mb-8 shadow-lg">
        <form onSubmit={handlePost} className="flex flex-col gap-3">
          <textarea value={newPost} onChange={e => setNewPost(e.target.value)} placeholder="Mande a resenha, cole o link do seu vídeo ou anexe uma foto..." className="w-full bg-slate-950 border border-slate-700 rounded-xl p-4 text-white placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500 outline-none resize-none min-h-[80px]" />
          
          {/* Mostra a miniatura da foto antes de enviar */}
          {postImage && (
            <div className="relative inline-block self-start mt-2">
              <img src={postImage} alt="Preview" className="h-32 rounded-lg border border-blue-500/50 shadow-md object-contain bg-black/50" />
          ...
          <div className="flex justify-between items-center mt-2 border-t border-slate-800/50 pt-3">
            <label className="cursor-pointer text-slate-400 hover:text-blue-400 flex items-center gap-2 transition-colors px-2 py-1 rounded-lg hover:bg-slate-800">
              <Camera size={20} /> <span className="text-sm font-bold">Anexar Foto</span>
              <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
            </label>
            <button type="submit" disabled={(!newPost.trim() && !postImage) || isPosting} className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 disabled:text-slate-500 text-white font-bold py-2 px-6 rounded-lg transition-colors flex items-center gap-2">
          ...
          return (
            <div key={post.id} className="bg-slate-900 rounded-2xl border border-slate-800 p-5 shadow-md relative group">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center overflow-hidden border border-blue-500/30 shrink-0">
                  {post.authorPhoto ? <img src={post.authorPhoto} alt="Foto" className="w-full h-full object-cover"/> : <span>👤</span>}
                </div>
                <div>
                  <p className="font-bold text-blue-400">{post.authorName}</p>
              ...
              <div className="bg-slate-950 rounded-xl p-3 space-y-3">
                {post.comments.map(c => (
                  <div key={c.id} className="text-sm border-b border-slate-800/50 pb-2 last:border-0 last:pb-0">
                    <span className="font-bold text-blue-400 mr-2">{c.authorName}:</span>
                    <span className="text-slate-300">{c.text}</span>
                  </div>
                ))}
                <div className="flex gap-2 mt-2 pt-1">
                  <input type="text" placeholder="Comente algo..." value={commentText[post.id] || ''} onChange={e => setCommentText({...commentText, [post.id]: e.target.value})} onKeyDown={e => e.key === 'Enter' && handleComment(post.id)} className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-blue-500" />
                  <button onClick={() => handleComment(post.id)} className="text-blue-500 hover:text-blue-400 font-bold px-2 text-sm"><Send size={18}/></button>
                </div>
              </div>
            </div>
          );
    ...
    <div className="animate-in fade-in duration-500 space-y-6">
      <div className="flex items-center gap-4 bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-lg">
        <label className="cursor-pointer relative group flex flex-col items-center shrink-0" title="Clique para trocar sua foto">
          <div className="relative w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center text-3xl border-2 border-blue-500/30 overflow-hidden shadow-lg">
        ...
        <div>
          ...
          <p className="text-blue-400 font-bold tracking-widest text-xs uppercase mt-1">{ROLE_NAMES[currentUser.role] || 'Membro'}</p>
        </div>
      </div>
          ...
          return (
            <div key={team.id} className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-xl">
              <div className="bg-slate-950/80 p-6 border-b border-slate-800 flex items-center gap-4">
               <label className="cursor-pointer relative group flex flex-col items-center" title="Clique para trocar o escudo">
                  <div className="relative">
                    ...
                    <div className="absolute -bottom-1 -right-2 bg-blue-600 rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 group-hover:scale-110 transition-all flex items-center justify-center">
              ...
              <div className="p-6 space-y-10">
                ...
                <div>
                  <h4 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><Trophy className="text-blue-500" size={20}/> Desempenho nos Torneios</h4>
                      ...
                      {activeComps.map(comp => {
                        ...
                        return (
                          <div key={comp.id} className="bg-slate-950 rounded-xl border border-slate-800 overflow-hidden">
                            <div className="bg-slate-900 p-3 border-b border-slate-800 flex justify-between items-center px-4"><span className="text-sm font-bold text-slate-200">{comp.name}</span><div className="flex items-center gap-2"><span className="text-[10px] uppercase font-bold text-slate-500 hidden sm:block">{comp.format === 'league' ? 'Liga' : 'Copa'}</span><span className="text-xs bg-blue-500/10 text-blue-400 px-2 py-1 rounded border border-blue-500/20 font-bold">{rank}º Lugar</span></div></div>
                            {myStats && myStats.p > 0 ? (
                              <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 p-4 text-center">
                                <div><p className="text-[10px] text-slate-500 uppercase font-bold mb-0.5">PTS</p><p className="text-xl font-black text-blue-400">{myStats.pts}</p></div>
                                ...
                                <div><p className="text-[10px] text-slate-500 uppercase font-bold mb-0.5">V</p><p className="text-lg font-bold text-blue-500">{myStats.w}</p></div>
                ...
                <div className="pt-4 border-t border-slate-800">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-800/50 text-center col-span-2 md:col-span-2"><p className="text-slate-500 text-xs mb-1 font-medium">Maior Goleada</p>{biggestWin ? ( <p className="text-lg font-bold text-white"><span className="text-blue-400">{biggestWin.scoreFor}</span> x {biggestWin.scoreAgainst} <span className="text-sm text-slate-400 font-normal">({teams.find(t=>t.id === biggestWin.oppId)?.name})</span></p> ) : <p className="text-sm text-slate-600 mt-1">Nenhuma vitória</p>}</div>
                  </div>
                </div>
...
const Dashboard = ({ matches, teams, competitions, currentUser, onSelectMatch }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="bg-gradient-to-r from-blue-900/50 to-slate-900 p-6 rounded-2xl border border-blue-900/50 shadow-xl">
      ...
      <div>
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2"><Activity size={20} className="text-blue-500" /> Últimos Resultados Enviados</h3>
        <div className="space-y-3">
          ...
          {recentMatches.map(m => {
            ...
            return (
              <div key={m.id} onClick={() => onSelectMatch && onSelectMatch(m)} className="bg-slate-900 p-3 md:p-4 rounded-xl border border-slate-800 flex flex-col gap-3 shadow-sm cursor-pointer hover:border-blue-500/50 hover:shadow-lg transition-all group relative">
                <div className="flex items-center justify-between w-full gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0 justify-start"><div className="shrink-0"><ShieldDisplay shield={tA?.shield} size="normal" /></div><span className="font-medium text-[11px] md:text-sm text-slate-200 truncate group-hover:text-blue-400 transition-colors">{String(tA?.name || 'Time A')}</span></div>
                  <div className="flex items-center justify-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 bg-slate-950 rounded-lg border border-slate-800 shrink-0">{m.penaltiesA !== null && m.penaltiesA !== undefined && <span className="text-[10px] text-amber-400 font-bold mr-1">({m.penaltiesA})</span>}<span className="font-bold text-sm md:text-base text-blue-400">{m.status === 'approved' || m.status === 'pending' ? String(m.scoreA) : '?'}</span><span className="text-[10px] md:text-xs text-slate-500 font-bold mx-0.5">X</span><span className="font-bold text-sm md:text-base text-blue-400">{m.status === 'approved' || m.status === 'pending' ? String(m.scoreB) : '?'}</span>{m.penaltiesB !== null && m.penaltiesB !== undefined && <span className="text-[10px] text-amber-400 font-bold ml-1">({m.penaltiesB})</span>}</div>
                  <div className="flex items-center gap-2 flex-1 min-w-0 justify-end"><span className="font-medium text-[11px] md:text-sm text-slate-200 truncate text-right group-hover:text-blue-400 transition-colors">{String(tB?.name || 'Time B')}</span><div className="shrink-0"><ShieldDisplay shield={tB?.shield} size="normal" /></div></div>
                </div>
                <div className="flex justify-center border-t border-slate-800/50 pt-2 flex-col items-center gap-1">{m.status === 'approved' ? <span className="text-[10px] uppercase font-bold tracking-wider text-blue-400">✅ Oficializado • Clique para detalhes</span> : <span className="text-[10px] uppercase font-bold tracking-wider text-amber-400 font-medium">⏳ Aguardando Validação • Clique para detalhes</span>}</div>
              </div>
            );
          })}
        </div>
        ...
        <div className="sticky top-0 bg-slate-900/95 backdrop-blur border-b border-slate-800 p-4 sm:p-6 flex justify-between items-center z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-blue-400 font-medium uppercase tracking-widest mt-1">Técnico: {team.coach}</p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 space-y-8">
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-center"><p className="text-slate-500 text-[10px] uppercase font-bold mb-1">Vitórias</p><p className="text-2xl font-bold text-blue-400">{wins}</p></div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 p-4 md:p-6 rounded-2xl border border-slate-800 shadow-xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-400 font-bold tracking-widest uppercase mt-0.5">{(teams || []).length} Times Cadastrados</p>
          </div>
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto">
          <input type="text" placeholder="Procurar time ou técnico..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="flex-1 md:w-64 bg-slate-950 border border-slate-700 focus:border-blue-500 rounded-lg p-2 text-white outline-none transition-colors text-sm" />
          <div className="flex p-1 bg-slate-950 rounded-lg border border-slate-700 shrink-0">
            <button onClick={() => setViewMode('grid')} className={`px-3 py-1.5 rounded-md transition-colors text-xs font-bold ${viewMode === 'grid' ? 'bg-slate-800 text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Grade</button>
            <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-md transition-colors text-xs font-bold ${viewMode === 'list' ? 'bg-slate-800 text-blue-400 shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Lista</button>
          </div>
        </div>
      </div>
        ...
        <div className={viewMode === 'grid' ? "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-4" : "flex flex-col gap-3"}>
          {filteredTeams.map(team => {
            ...
            if (editingId === team.id) {
              return (
                <div key={safeTeamId} className={`bg-slate-900 p-3 rounded-xl border border-blue-500/50 shadow-[0_0_15px_rgba(16,185,129,0.1)] flex ${viewMode === 'list' ? 'flex-col md:flex-row items-start md:items-center justify-between gap-4' : 'flex-col justify-between gap-3'}`}>
                  <div className={`flex items-center gap-2 ${viewMode === 'list' ? 'flex-row w-full flex-wrap' : 'flex-col'}`}>
                    <div className="shrink-0 pt-1">
                      <label className="cursor-pointer relative group flex flex-col items-center">
                        <div className="relative">
                          ...
                          <div className="absolute -bottom-1 -right-2 bg-blue-600 rounded-full p-1 shadow-lg group-hover:scale-110 transition-transform flex items-center justify-center"><UploadCloud size={10} className="text-white" /></div>
                        </div>
                    ...
                    <div className={`flex-1 space-y-1.5 w-full ${viewMode === 'list' ? 'grid grid-cols-2 sm:grid-cols-4 gap-2 space-y-0 mt-0' : 'mt-1'}`}>
                      <input type="text" value={editData.name} onChange={e=>setEditData({...editData, name: e.target.value})} placeholder="Time" className="w-full bg-slate-950 border border-slate-700 rounded p-1.5 text-white text-[10px] md:text-xs outline-none focus:border-blue-500" />
                      <input type="text" value={editData.coach} onChange={e=>setEditData({...editData, coach: e.target.value})} placeholder="Técnico" className="w-full bg-slate-950 border border-slate-700 rounded p-1.5 text-white text-[10px] md:text-xs outline-none focus:border-blue-500" />
                      <input type="text" value={editData.whatsapp} onChange={e=>setEditData({...editData, whatsapp: e.target.value})} placeholder="WhatsApp" className="w-full bg-slate-950 border border-slate-700 rounded p-1.5 text-white text-[10px] md:text-xs outline-none focus:border-blue-500" />
                      ...
                      }} className="w-full bg-slate-950 border border-slate-700 rounded p-1.5 text-white text-[10px] md:text-xs outline-none focus:border-blue-500">
            ...
            if (viewMode === 'list') {
               return (
                <div key={safeTeamId} onClick={() => setViewingTeam(team)} className="relative bg-slate-900 p-3 sm:p-4 rounded-xl border border-slate-800 hover:border-blue-500/50 hover:shadow-lg transition-all flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group cursor-pointer">
                  <div className="flex items-center gap-4 flex-1 min-w-0 w-full">
                    <div className="shrink-0"><ShieldDisplay shield={team.shield} size="normal" /></div>
                    <div className="flex-1 min-w-0 pr-10 sm:pr-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-sm md:text-base font-bold text-white leading-tight truncate group-hover:text-blue-400 transition-colors">{String(team.name || 'Time')}</h3>
                  ...
                  <div className="absolute top-2 right-2 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-10">
                    <button onClick={(e) => { e.stopPropagation(); startEdit(team); }} className="text-slate-500 hover:text-blue-400 p-1.5 rounded-lg hover:bg-slate-800" title="Editar"><Edit size={14} /></button> 
                  ...
                  <Button onClick={(e) => { e.stopPropagation(); handleWhatsApp(team.whatsapp); }} className="w-full sm:w-auto bg-blue-600 hover:bg-blue-500 text-white py-2 px-3 text-xs disabled:bg-slate-800 disabled:text-slate-500 shrink-0 z-10" disabled={!team.whatsapp}>
            ...
            return (
              <div key={safeTeamId} onClick={() => setViewingTeam(team)} className="relative bg-slate-900 p-3 md:p-4 rounded-xl border border-slate-800 hover:border-blue-500/50 hover:shadow-lg transition-all flex flex-col justify-between gap-3 group cursor-pointer">
                {isAdmin && ( 
                    <div className="absolute top-3 sm:top-auto sm:relative right-3 sm:right-auto flex gap-1 sm:opacity-0 sm:group-hover:opacity-100 shrink-0 z-10">
                      <button onClick={(e) => { e.stopPropagation(); startEdit(team); }} className="text-slate-500 hover:text-blue-400 p-1.5 rounded-lg hover:bg-slate-800 transition-colors" title="Editar"><Edit size={16} /></button>
                ...
                <div className="flex flex-col items-center text-center gap-2 mt-2">
                  ...
                  <div className="w-full">
                    <h3 className="text-sm md:text-base font-bold text-white leading-tight truncate px-2 group-hover:text-blue-400 transition-colors">{String(team.name || 'Time')}</h3>
                ...
                <Button onClick={(e) => { e.stopPropagation(); handleWhatsApp(team.whatsapp); }} className="w-full bg-blue-600 hover:bg-blue-500 text-white mt-1 py-1.5 text-[10px] md:text-xs px-2 disabled:bg-slate-800 disabled:text-slate-500 z-10" disabled={!team.whatsapp}>
            ...
            {Object.keys(comp.groups || {}).map((gName, idx) => {
              ...
              return (
                <div key={gName} className={idx > 0 ? "border-t-4 border-slate-950" : ""}>
                  <div className="bg-slate-800/50 p-3 text-center border-b border-slate-800 flex justify-between px-4"><h3 className="text-sm font-bold text-blue-400 uppercase tracking-widest">Grupo {gName}</h3></div>
                  <table className="w-full text-left text-sm whitespace-nowrap"><thead className="bg-slate-950/50 text-slate-400 font-medium"><tr><th className="p-4 w-12 text-center">#</th><th className="p-4">Time</th><th className="p-4 text-center">PTS</th><th className="p-4 text-center">J</th><th className="p-4 text-center">V</th><th className="p-4 text-center">E</th><th className="p-4 text-center">D</th><th className="p-4 text-center">GP</th><th className="p-4 text-center">GC</th><th className="p-4 text-center">SG</th></tr></thead>
                    <tbody className="divide-y divide-slate-800/50">
                      {gTable.map((row, index) => {
                        ...
                        const borderClass = isQualified ? 'border-l-4 border-blue-500' : (isBottom ? 'border-l-4 border-red-500' : 'border-l-4 border-transparent');
                        const bgClass = isQualified ? 'bg-blue-500/5' : (isBottom ? 'bg-red-500/5' : '');
                        const textNumberClass = isQualified ? 'text-blue-400' : (isBottom ? 'text-red-400' : 'text-slate-500');
                        ...
                        return (
                          <tr key={row.id} className={`hover:bg-slate-800/50 transition-colors ${borderClass} ${bgClass}`}>
                            ...
                            <td className="p-4 text-center font-bold text-blue-400">{row.pts}</td><td className="p-4 text-center text-slate-300">{row.p}</td><td className="p-4 text-center text-slate-300">{row.w}</td><td className="p-4 text-center text-slate-300">{row.d}</td><td className="p-4 text-center text-slate-300">{row.l}</td><td className="p-4 text-center text-slate-400">{row.gf}</td><td className="p-4 text-center text-slate-400">{row.ga}</td><td className="p-4 text-center text-slate-400 font-medium">{row.gd > 0 ? `+${row.gd}` : row.gd}</td>
                          </tr>
                        );
                      })}
                    </tbody>
            ...
            <tbody className="divide-y divide-slate-800/50">
              {(() => {
                ...
                return displayTable.map((row, index) => {
                  ...
                  // Lógica das Barrinhas e Cores
                  const borderClass = isTop ? 'border-l-4 border-blue-500' : (isBottom ? 'border-l-4 border-red-500' : 'border-l-4 border-transparent');
                  const bgClass = isTop ? 'bg-blue-500/5' : (isBottom ? 'bg-red-500/5' : '');
                  const textNumberClass = isTop ? 'text-blue-400' : (isBottom ? 'text-red-400' : 'text-slate-500');
...
const CompetitionDetails = ({ comp, teams, matches, onBack, currentUser, onReleaseRound, onSelectMatch, onDeleteMatch, onEditComp, showToast, onUpdatePlayedMatch }) => {
  ...
  if (!comp) return (<div className="text-center py-12"><p className="text-slate-400">Torneio não localizado.</p><button onClick={onBack} className="text-blue-400 underline">Voltar</button></div>);
  ...
  const getMatchStatusDisplay = (matchId) => {
    ...
    if(sm.status === 'approved') return { submittedMatchId: sm.id, isPlayed: true, scoreA: sm.scoreA, scoreB: sm.scoreB, penaltiesA: sm.penaltiesA, penaltiesB: sm.penaltiesB, text: 'Oficial', color: 'text-blue-400', bg: 'bg-slate-950 border-blue-900/50' };
    ...
    <div className="space-y-6 animate-in fade-in pb-10">
      ...
      <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-blue-400 mt-1 uppercase font-bold">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            {showAddTeam ? (
              ...
              <Button variant="outline" onClick={()=>setShowAddTeam(true)} className="py-2 px-3 text-xs w-full md:w-auto flex items-center justify-center gap-2">
                <span className="text-blue-400 font-bold">+</span> Inserir Time
              </Button>
            )}
          </div>
        )}
      ...
      <div className="flex gap-1 p-1 bg-slate-950 rounded-xl border border-slate-800">
        <button onClick={()=>setSubTab('overview')} className={`flex-1 py-1.5 text-xs rounded-lg font-bold transition-all ${subTab==='overview'?'bg-blue-600 text-white shadow-md':'text-slate-500 hover:text-white'}`}>Tabela & Jogos</button>
        <button onClick={()=>setSubTab('stats')} className={`flex-1 py-1.5 text-xs rounded-lg font-bold transition-all ${subTab==='stats'?'bg-blue-600 text-white shadow-md':'text-slate-500 hover:text-white'}`}>Estatísticas</button>
      </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-left-4">
            ...
            <div className="space-y-2">
              ...
              <div id="capture-standings" className="bg-slate-950 p-3 sm:p-5 rounded-2xl border border-slate-800">
                <h3 className="text-center font-black text-blue-400 mb-4 text-sm uppercase tracking-widest">{comp.name}</h3>
            ...
            <div className="space-y-3 pt-4 border-t border-slate-800/50">
              ...
              {(comp.rounds || []).map((round) => {
                ...
                return (
                  <div key={round?.id} className={`bg-slate-900 border ${isExpanded ? 'border-blue-500/50 shadow-lg' : 'border-slate-800 hover:border-slate-700'} rounded-xl overflow-hidden transition-all`}>
                    <button onClick={() => toggleRound(round.id)} className="w-full bg-slate-950/60 p-4 flex justify-between items-center transition-colors outline-none">
                      <span className={`text-sm font-bold flex items-center gap-2 ${isExpanded ? 'text-blue-400' : 'text-white'}`}>
                        {round.status === 'locked' ? <Lock size={16} className="text-slate-500"/> : <PlayCircle size={16} className="text-blue-500"/>}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`font-bold transition-transform duration-300 ${isExpanded ? 'text-blue-400 rotate-180' : 'text-slate-500'}`}>▼</span>
                      </div>
                    </button>
                    ...
                    {isExpanded && (
                      <div className="bg-slate-900 border-t border-slate-800 animate-in slide-in-from-top-2 p-3">
                        ...
                        <div id={`capture-round-${round.id}`} className="bg-slate-950 p-4 rounded-xl border border-slate-800/50 space-y-3">
                          <h4 className="text-center font-bold text-white mb-4 text-xs uppercase tracking-widest">{comp.name} <span className="text-blue-400">• Rodada {round.number}</span></h4>
                          ...
                          {(round?.matches || []).map((m) => {
                            ...
                            if (editMatchData?.id === m.id) {
                              return (
                                <div key={m.id} className="bg-slate-900 p-3 rounded-lg border border-blue-500/50 flex flex-col gap-3 shadow-lg">
                                  <div className="flex items-center gap-2">
                            ...
                            return (
                              <div key={m.id} className="relative group">
                                ...
                                {isAdmin && (
                                  <button onClick={(e) => { e.stopPropagation(); setEditMatchData({ ...m, roundId: round.id }); }} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-blue-400 p-2 bg-slate-950 rounded-lg opacity-0 group-hover:opacity-100 transition-all z-10" title="Editar Confronto">
                                    <Edit size={14} />
                                  </button>
                                )}
                              </div>
            ...
            <div className="space-y-2">
              ...
              <div id="capture-scorers" className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-xl p-2 sm:p-4">
                <div className="bg-slate-950/80 p-4 border border-slate-800 rounded-xl mb-4 flex flex-col items-center justify-center">
                  <h3 className="font-bold text-blue-400 text-lg uppercase tracking-widest text-center">⚽ Artilharia</h3>
                ...
                <div className="divide-y divide-slate-800/50 bg-slate-950 rounded-xl border border-slate-800">
                  {topScorers.length === 0 ? <p className="p-6 text-sm text-slate-500 text-center">Nenhum gol validado até o momento.</p> : topScorers.map((s, idx) => (
                    <div key={idx} className="p-3 flex items-center justify-between hover:bg-slate-800/50 transition-colors">
                      ...
                      <div className="bg-slate-900 px-4 py-2 rounded-lg border border-slate-800 text-blue-400 font-black text-lg">{s.count}</div>
                    </div>
                  ))}
                </div>
              </div>
...
const CreateCompetition = ({ teams, currentUser, onCreate }) => {
  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in pb-12">
      <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2"><PlusCircle className="text-blue-500"/> Nova Competição</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-800">
          <h3 className="text-lg font-bold text-blue-400 mb-4 flex items-center gap-2"><Trophy size={18}/> Estrutura do Torneio</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2"><label className="text-sm font-medium text-slate-400">Nome do Campeonato</label><input type="text" placeholder="Ex: Liga de Inverno" value={name} onChange={e=>setName(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" required /></div>
            <div className="space-y-2"><label className="text-sm font-medium text-slate-400">Formato</label>
              <select value={format} onChange={e=>setFormat(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none">
            ...
            <div className="space-y-2"><label className="text-sm font-medium text-slate-400">Qtd. de Times</label><input type="number" min="2" placeholder="Ex: 8" value={teamCount} onChange={e=>setTeamCount(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none" required /></div>
            <div className="space-y-2"><label className="text-sm font-medium text-slate-400">Prazo de Conclusão</label><input type="date" value={deadline} onChange={e=>setDeadline(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-slate-300 focus:ring-2 focus:ring-blue-500 outline-none" required /></div>
            ...
            {format !== 'cup' && (
              <div className="space-y-2 flex items-center gap-2 mt-4 col-span-1 md:col-span-2 bg-slate-950 p-4 rounded-lg border border-slate-800">
                <input type="checkbox" id="isDoubleRound" checked={isDoubleRound} onChange={e=>setIsDoubleRound(e.target.checked)} className="w-5 h-5 accent-blue-500 cursor-pointer" />
            ...
            {format === 'groups' && (
              <><div className="space-y-2"><label className="text-sm font-medium text-slate-400">Quantidade de Grupos</label>
                  <select value={numGroups} onChange={e=>setNumGroups(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none"><option value="2">2 Grupos</option><option value="4">4 Grupos</option><option value="8">8 Grupos</option></select>
                </div><div className="space-y-2"><label className="text-sm font-medium text-slate-400">Classificados por Grupo</label>
                  <select value={qualifiers} onChange={e=>setQualifiers(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none"><option value="1">1 Time</option><option value="2">2 Times</option><option value="4">4 Times</option></select>
                </div></>
            )}
          </div>
        </div>
          ...
          {teams.length === 0 ? <p className="text-slate-500 text-sm p-4 bg-slate-950 rounded border border-slate-800">Nenhum time cadastrado.</p> : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {teams.map(team => { 
                ...
                return ( 
                  <div key={team.id} onClick={() => toggleTeam(team.id)} className={`cursor-pointer flex items-center gap-3 p-3 rounded-xl border transition-all ${isSelected ? 'bg-blue-500/10 border-blue-500 shadow-[0_0_10px_rgba(16,185,129,0.1)]' : 'bg-slate-950 border-slate-800 hover:border-slate-600'}`}>
                    <ShieldDisplay shield={team.shield} size="small" />
                    <span className={`font-medium text-sm truncate ${isSelected ? 'text-blue-400' : 'text-slate-300'}`}>{team.name}</span>
                  </div> 
                ); 
              })}
            </div>
...
const CompetitionsList = ({ competitions, teams, currentUser, onSelectComp, onDeleteComp }) => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in">
      <div className="flex items-center gap-2 mb-4"><Medal className="text-blue-500"/><h2 className="text-xl font-bold text-white">Campeonatos Ativos</h2></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {visible.map(c => (
          <div key={c.id} onClick={()=>onSelectComp(c.id)} className="bg-slate-900 p-5 rounded-2xl border border-slate-800 hover:border-blue-500/40 transition-all cursor-pointer flex justify-between items-center group">
            <div><h3 className="font-bold text-white group-hover:text-blue-400 transition-colors">{String(c.name)}</h3><p className="text-xs text-slate-500 mt-1">{c.teams?.length || 0} Clubes inscritos</p></div>
...
const MatchDetails = ({ match, teams, competitions, onBack }) => {
  ...
  return (
    <div className="max-w-xl mx-auto space-y-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl animate-in fade-in">
      ...
      <div className="text-center font-mono border-b border-slate-800 pb-3"><span className="text-blue-400 text-2xl font-black">{match.scoreA} x {match.scoreB}</span><div className="text-xs text-slate-400 mt-1">{tA?.name} vs {tB?.name}</div></div>
  ...
  return (
    <div className="max-w-2xl mx-auto animate-in fade-in duration-500 pb-12">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Camera className="text-blue-500" /> Registrar Partida</h2>
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 space-y-6">
        ...
        {showKeyInput && (
          <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-xl animate-in slide-in-from-top-4">
            ...
            <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-[10px] text-blue-400 hover:underline mt-2 inline-block">Clique aqui para gerar uma chave grátis ➔</a>
          </div>
        )}

        <div>
          <label className="block text-sm font-medium text-slate-400 mb-2">1. Competição</label>
          <select value={selectedCompId} onChange={e => setSelectedCompId(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none">
        ...
        {selectedCompId && (
          <div className="animate-in fade-in">
            ...
            {availableMatches.length > 0 ? (
              <select value={selectedMatchId} onChange={e => setSelectedMatchId(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:ring-2 focus:ring-blue-500 outline-none">
        ...
        {selectedMatchId && !isManualMode && (
          <div className="animate-in slide-in-from-top-4">
            ...
            <div className="mb-2">
              <label className={`border-2 border-dashed rounded-xl p-8 text-center transition-colors cursor-pointer relative overflow-hidden block ${matchImageBase64 ? 'border-blue-500 bg-blue-500/5' : 'border-slate-700 hover:border-slate-500 bg-slate-950'}`}>
                ...
                {isAnalyzing ? (
                  <div className="flex flex-col items-center space-y-3">
                    ...
                    <p className="text-blue-400 font-medium">IA analisando o print...</p>
                  </div>
                ) : imageUploaded ? (
                  <div className="flex flex-col items-center space-y-2">
                    <CheckCircle className="text-blue-500" size={40} />
                    <p className="text-blue-400 font-medium">Dados extraídos com sucesso!</p>
                  </div>
                ) : (
                  <div className="flex flex-col items-center space-y-3">
            ...
            {!imageUploaded && !isAnalyzing && (
              <div className="text-center mt-4">
                <button type="button" onClick={() => setIsManualMode(true)} className="text-sm text-slate-400 hover:text-blue-400 transition-colors underline">
          ...
          <form onSubmit={handleSubmit} className="animate-in slide-in-from-bottom-4 space-y-6 pt-4 border-t border-slate-800">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start bg-slate-950 p-4 rounded-xl border border-slate-800">
              <div className="flex-1 w-full space-y-3">
                ...
                <input type="number" value={scoreA} onChange={e=>setScoreA(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white text-center text-3xl font-bold focus:border-blue-500 outline-none" required />
                  ...
                  {goalsA.map((g, i) => (
                    <div key={i} className="flex flex-col gap-1 bg-slate-800 p-2 rounded">
                      ...
                      <div className="flex gap-1">
                        ...
                        <input type="number" value={g.minute} onChange={e=>handleGoalChange('A', i, 'minute', e.target.value)} placeholder="Min" className="w-12 bg-slate-950 text-xs text-blue-400 text-center px-1 py-1 rounded border border-slate-700 outline-none" required />
                        <button type="button" onClick={()=>handleRemoveGoal('A', i)} className="text-red-400 p-1 hover:text-red-300"><X size={12}/></button>
                      </div>
                    </div>
                  ))}
                  <button type="button" onClick={()=>handleAddGoal('A')} className="text-[10px] text-blue-400 hover:underline">+ Adicionar Gol</button>
                </div>
              </div>
              ...
              <div className="flex-1 w-full space-y-3">
                ...
                <input type="number" value={scoreB} onChange={e=>setScoreB(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-lg p-3 text-white text-center text-3xl font-bold focus:border-blue-500 outline-none" required />
                ...
                <div className="space-y-2 pt-2">
                  ...
                  {goalsB.map((g, i) => (
                    <div key={i} className="flex flex-col gap-1 bg-slate-800 p-2 rounded">
                      ...
                      <div className="flex gap-1">
                        ...
                        <input type="number" value={g.minute} onChange={e=>handleGoalChange('B', i, 'minute', e.target.value)} placeholder="Min" className="w-12 bg-slate-950 text-xs text-blue-400 text-center px-1 py-1 rounded border border-slate-700 outline-none" required />
                  ...
                  <div className="flex justify-end">
                    <button type="button" onClick={()=>handleAddGoal('B')} className="text-[10px] text-blue-400 hover:underline">+ Adicionar Gol</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <textarea placeholder="Ocorreu alguma queda de conexão? Relate aqui..." value={observacoes} onChange={e=>setObservacoes(e.target.value)} className="w-full bg-slate-950 border border-slate-700 focus:border-blue-500 rounded-lg p-3 text-slate-300 text-sm h-24 outline-none resize-none transition-colors" />
            </div>
            ...
            return (
              <div key={m.id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-3">
                ...
                <div className="flex items-center justify-between text-xs bg-slate-950 p-3 rounded-lg">
                  ...
                  <span className="px-3 py-1 font-mono font-black text-blue-400 bg-slate-900 border border-slate-800 rounded mx-2">{m.scoreA} x {m.scoreB}</span>
...
const CreateTeamManual = ({ onCreate, showToast }) => {
  ...
  return (
    <form onSubmit={async (e)=>{e.preventDefault(); if(!name)return; await onCreate({id:`t${Date.now()}`,name,coach:coach||'Técnico',whatsapp:'',ownerId:'manual',shield:shield||'🛡️'}); showToast("Time salvo!"); setName(''); setCoach(''); setShield(null); }} className="max-w-xl mx-auto bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 animate-in fade-in">
      ...
      <div className="bg-slate-950 p-3 rounded-xl flex items-center justify-between"><span className="text-xs text-slate-400">Escudo do Time:</span><label className="cursor-pointer bg-slate-800 px-3 py-1.5 rounded text-xs text-white hover:bg-blue-600"><UploadCloud size={14} className="inline mr-1"/> Enviar Imagem<input type="file" accept="image/*" className="hidden" onChange={e=>processImage(e.target.files[0],setShield)}/></label></div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
                ...
                return (
                  <tr key={u.id} className="hover:bg-slate-950/40">
                    ...
                    <td className="p-3 flex justify-center gap-2">
                      ...
                      <button onClick={()=>onApproveUser(u.id)} className="bg-blue-500/10 text-blue-400 px-3 py-1.5 rounded hover:bg-blue-500/20 transition-colors">Aprovar</button>
                    </td>
                  </tr>
                )
              })}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 border-b border-slate-800 flex items-center gap-2"><Award className="text-blue-500"/><h2 className="font-bold text-white text-base">Gestão de Elenco / Técnicos</h2></div>
        <div className="overflow-x-auto">
          ...
          <tbody className="divide-y divide-slate-800/40">
            {activeUsers.map(u=>{ 
              ...
              if (editingId === u.id) {
                return (
                  <tr key={u.id} className="bg-slate-950/80">
                    <td className="p-3"><input type="text" value={editData.name} onChange={e=>setEditData({...editData, name: e.target.value})} className="bg-slate-900 border border-slate-700 rounded p-1 text-white w-full outline-none focus:border-blue-500" /></td>
                    <td className="p-3 text-blue-400 font-medium">{t?.name || 'S/ Clube'}</td>
                    <td className="p-3"><input type="text" value={editData.whatsapp} onChange={e=>setEditData({...editData, whatsapp: e.target.value})} className="bg-slate-900 border border-slate-700 rounded p-1 text-white w-full outline-none focus:border-blue-500" /></td>
                    ...
                    <td className="p-3 flex justify-center gap-2">
                      ...
                      <button onClick={()=>saveEdit(u)} className="bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-500 shadow-lg transition-colors">Salvar</button>
                    </td>
                  </tr>
                );
              }
              ...
              return(
                <tr key={u.id} className="hover:bg-slate-950/40">
                  <td className="p-3 font-bold text-slate-200">{u.name}</td><td className="p-3 text-blue-400 font-medium">{t?.name || 'S/ Clube'}</td><td className="p-3 font-mono text-slate-400">{u.whatsapp}</td>
                  ...
                  <td className="p-3 flex justify-center gap-3 items-center">
                    {isSupremeLeader && <button onClick={()=>startEdit(u)} className="text-slate-500 hover:text-blue-400 transition-colors p-1" title="Editar Técnico"><Edit size={16}/></button>}
 ...
 const TABS = [
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans flex flex-col md:flex-row relative">
      {toastMessage && (
        <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-top-4 ${toastMessage.type === 'error' ? 'bg-red-950 border border-red-500 text-red-100' : 'bg-slate-800 border border-blue-500 text-white'}`}>
          {toastMessage.type === 'error' ? <AlertCircle className="text-red-500" size={20} /> : <CheckCircle className="text-blue-500" size={20} />}
      ...
      <aside className="w-full md:w-64 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-800 flex flex-col shrink-0 z-10 shadow-2xl">
        <div className="p-6 flex items-center gap-3"><img src={LOGO_URL} alt="Clã Kame" className="w-10 h-10" /><div><h1 className="font-bold text-white text-lg">Clã Kame</h1><p className="text-[10px] text-blue-400 font-bold uppercase">Arena DLS</p></div></div>
        <nav className="flex-1 px-4 pb-4 overflow-y-auto flex md:flex-col gap-2 overflow-x-auto custom-scrollbar">
          {TABS.map(tab => {
            ...
            return ( <button key={tab.id} onClick={() => setCurrentTab(tab.id)} className={`flex items-center gap-3 px-4 py-3 rounded-xl whitespace-nowrap outline-none border ${isActive ? 'bg-blue-500/10 text-blue-400 font-bold border-blue-500/20' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-200 border-transparent'}`}><Icon size={18} /> <span className="text-sm">{tab.label}</span>{(tab.id === 'validation' && matches.filter(m=>m?.status==='pending').length > 0) && <span className="ml-auto bg-amber-500 text-slate-950 text-xs font-bold px-2 py-0.5 rounded-full">{matches.filter(m=>m?.status==='pending').length}</span>}</button> );
          })}
        </nav>
        <div className="p-4 border-t border-slate-800 hidden md:block"><div className="bg-slate-950 rounded-xl p-4 border border-slate-800/50 relative overflow-hidden"><div className="absolute top-0 left-0 w-1 h-full bg-blue-500"></div><p className="font-bold text-white text-sm truncate">{String(currentUser?.name)}</p><p className="text-[10px] text-blue-400 uppercase font-bold mb-3">{ROLE_NAMES[currentUser?.role]}</p><button onClick={() => { setCurrentUser(null); signOut(auth); }} className="w-full text-xs text-slate-400 hover:text-white py-1.5 rounded bg-slate-900 border border-slate-700/60"><LogOut size={12} className="inline mr-1"/> Sair</button></div></div>
      </aside>
