...
const SubmitMatch = ({ teams, competitions, matches, onSubmit, currentUser, showToast }) => {
  ...
  const resetAI = () => { setScoreA(''); setScoreB(''); setPenaltiesA(''); setPenaltiesB(''); setGoalsA([]); setGoalsB([]); setObservacoes(''); setImageUploaded(false); setMatchImageBase64(null); };
  ...
  const handleImageUpload = (e) => {
    ...
    processScreenshot(file, async (base64) => {
      setMatchImageBase64(base64); setIsAnalyzing(true); setImageUploaded(false); setScoreA(''); setScoreB(''); setGoalsA([]); setGoalsB([]); setPenaltiesA(''); setPenaltiesB('');
      try {
        ...
        {selectedMatchId && !imageUploaded && (
          <div className="animate-in slide-in-from-top-4">
            ...
            <div className="mb-6 space-y-4">
              ...
              <Button type="button" variant="outline" onClick={() => { setImageUploaded(true); setMatchImageBase64(null); }} className="w-full py-4 border-slate-700 border-dashed text-slate-400 hover:text-white hover:bg-slate-800 hover:border-slate-500">
                <Edit size={18} /> Preencher Manualmente (Sem Print)
              </Button>
            </div>
          </div>
        )}
        ...
        {imageUploaded && (
          <form onSubmit={handleSubmit} className="animate-in slide-in-from-bottom-4 space-y-6 pt-4 border-t border-slate-800">
            <div className="flex justify-between items-center mb-2">
              ...
              <button type="button" onClick={resetAI} className="text-xs text-slate-500 hover:text-slate-300 flex items-center gap-1 transition-colors">
